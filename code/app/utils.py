...
def get_and_insert_clubs(db,models):
    pagesize=40
    pages = 2
        ...
        if response.status_code == 200:
            ...
            for datum in data:
                ...
                db.session.add(models.Club(
                    id=attribs['field_club_cms_identifier'],
                    name=attribs['field_easy_solution_club_id'],
                    title=attribs['title'],
                    address=f"{attribs['field_address']['address_line1']}, {attribs['field_address']['postal_code']} {attribs['field_address']['locality']}"))
                db.session.commit()


def seed_data(db, models):
    if models.Club.query.first() is None:
        get_and_insert_clubs(db,models)
